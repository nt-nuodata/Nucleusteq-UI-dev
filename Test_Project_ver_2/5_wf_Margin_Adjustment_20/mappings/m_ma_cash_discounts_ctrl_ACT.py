...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_CASH_DISCOUNT_CTRL_1
...
df_1.createOrReplaceTempView("Shortcut_to_MA_CASH_DISCOUNT_CTRL_1")
...
FROM
  Shortcut_to_MA_CASH_DISCOUNT_CTRL_1""")
...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_MONTHS_4
...
df_4.createOrReplaceTempView("Shortcut_to_MONTHS_4")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_VENDOR_PROFILE_RPT_5
...
df_5.createOrReplaceTempView("Shortcut_to_VENDOR_PROFILE_RPT_5")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_SALES_DAY_SKU_STORE_RPT_6
...
df_6.createOrReplaceTempView("Shortcut_to_SALES_DAY_SKU_STORE_RPT_6")
...
df_9 = spark.sql("""SELECT
  SLS.FISCAL_MO AS FISCAL_MO,
  SLS.VENDOR_ID AS VENDOR_ID,
  SLS.ACT_NET_SALES_COST AS ACT_NET_SALES_COST,
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  (
    SELECT
    ...
    FROM
      Shortcut_to_SALES_DAY_SKU_STORE_RPT_6 S
      JOIN Shortcut_To_DAYS_8 D ON S.DAY_DT = D.DAY_DT
...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_GL_ACTUAL_DETAIL_11
...
df_11.createOrReplaceTempView("Shortcut_to_GL_ACTUAL_DETAIL_11")
...
df_12 = spark.sql("""SELECT
  GL.FISCAL_MO AS FISCAL_MO,
  GL.VENDOR_ID AS VENDOR_ID,
  GL.ACT_CASH_DISCOUNT_GL_AMT AS ACT_CASH_DISCOUNT_GL_AMT,
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  (
    SELECT
    ...
    FROM
      (
        ...
        FROM
          Shortcut_to_GL_ACTUAL_DETAIL_11 GAD
          JOIN SITE_PROFILE_RPT SITE ON GAD.LOCATION_ID = SITE.LOCATION_ID
          ...
          JOIN (
            ...
            FROM
              Shortcut_to_GL_ACTUAL_DETAIL_11 GAD
              JOIN GL_ACCOUNT_PROFILE GAP ON GAD.GL_ACCOUNT_GID = GAP.GL_ACCOUNT_GID
              JOIN MONTHS m ON GAD.FISCAL_MO = M.FISCAL_MO
              JOIN MA_FISCAL_MO_CTRL MFMC ON M.FISCAL_MO = MFMC.FISCAL_MO
              AND MFMC.RESTATE_DT = CURRENT_DATE
              JOIN (
                SELECT
              ...
              ) METVC ON GAP.GL_ACCOUNT_NBR:: VARCHAR (25) = METVC.MA_EVENT_TYPE_VAR_VALUE
              AND GAD.GL_POSTING_DT BETWEEN METVC.START_EFF_DT
              AND METVC.END_EFF_DT
          ) pre ON gad.FISCAL_MO = pre.FISCAL_MO
      ...
      JOIN VENDOR_PROFILE_RPT V ON CAST(P.VENDOR_ID AS VARCHAR (50)) = V.VENDOR_NBR
      AND V.VENDOR_TYPE_ID IN (2)
    GROUP BY
      P.FISCAL_MO,
      V.VENDOR_ID
  ) GL""")
...
FROM
  ...
  LEFT JOIN SQ_SALES_QUERY_9 DETAIL ON MASTER.FISCAL_MO = DETAIL.FISCAL_MO
  AND MASTER.VENDOR_ID = DETAIL.VENDOR_ID""")
...
FROM
  ...
  LEFT JOIN SQ_Shortcut_to_MA_CASH_DISCOUNT_CTRL_2 DETAIL ON MASTER.FISCAL_MO1 = DETAIL.FISCAL_MO
  AND MASTER.VENDOR_ID1 = DETAIL.SOURCE_VENDOR_ID""")
...
df_15 = spark.sql("""SELECT
  FISCAL_MO AS FISCAL_MO,
  VENDOR_ID1 AS SOURCE_VENDOR_ID,
  ACT_CASH_DISCOUNT_GL_AMT AS ACT_CASH_DISCOUNT_GL_AMT,
  ACT_NET_SALES_COST AS ACT_NET_SALES_COST,
  IFF(
    IFF(ISNULL(ACT_NET_SALES_COST), 0, ACT_NET_SALES_COST) = 0,
    0,
    ACT_CASH_DISCOUNT_GL_AMT / ACT_NET_SALES_COST
  ) AS THIS_ACT_CASH_DISCOUNT_PCT,
  now() AS UPDATE_TSTMP,
  ...
  IFF(
    ...
    IFF(
      IFF(
        ...
        ISNULL(ACT_CASH_DISCOUNT_PCT),
        0,
        ACT_CASH_DISCOUNT_PCT
      ),
      'U',
      'X'
    )
...
USING
  UPD_STRATEGY_17 AS SOURCE ON TARGET.SOURCE_VENDOR_ID = SOURCE.SOURCE_VENDOR_ID
...
SET
  ...
  TARGET.ACT_CASH_DISCOUNT_GL_AMT = SOURCE.ACT_CASH_DISCOUNT_GL_AMT,
  TARGET.ACT_CASH_DISCOUNT_PCT = SOURCE.THIS_ACT_CASH_DISCOUNT_PCT,
  TARGET.UPDATE_TSTMP = SOURCE.UPDATE_TSTMP,
  TARGET.LOAD_TSTMP = SOURCE.LOAD_TSTMP
  WHEN MATCHED
  AND SOURCE.UPDATE_STRATEGY_FLAG = "DD_DELETE"
  AND TARGET.ACT_NET_SALES_COST = SOURCE.ACT_NET_SALES_COST
  AND TARGET.ACT_CASH_DISCOUNT_GL_AMT = SOURCE.ACT_CASH_DISCOUNT_GL_AMT
  AND TARGET.ACT_CASH_DISCOUNT_PCT = SOURCE.THIS_ACT_CASH_DISCOUNT_PCT
...
INSERT
  (
    ...
    TARGET.ACT_CASH_DISCOUNT_GL_AMT,
    TARGET.ACT_CASH_DISCOUNT_PCT,
    TARGET.UPDATE_TSTMP,
    TARGET.LOAD_TSTMP
  )
VALUES
  (
    ...
    SOURCE.ACT_CASH_DISCOUNT_GL_AMT,
    SOURCE.THIS_ACT_CASH_DISCOUNT_PCT,
    SOURCE.UPDATE_TSTMP,
    SOURCE.LOAD_TSTMP
  )""")