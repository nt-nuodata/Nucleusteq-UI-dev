...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_SERVICES_MARGIN_WAGE_TYPE_CTRL_0
...
df_0.createOrReplaceTempView("Shortcut_to_SERVICES_MARGIN_WAGE_TYPE_CTRL_0")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_SERVICES_MARGIN_CTRL_1
...
df_1.createOrReplaceTempView("Shortcut_to_SERVICES_MARGIN_CTRL_1")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_EMPL_EMPL_LOC_WK_2
...
df_2.createOrReplaceTempView("Shortcut_to_EMPL_EMPL_LOC_WK_2")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_EARNINGS_ID_3
...
df_3.createOrReplaceTempView("Shortcut_to_EARNINGS_ID_3")
...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_SITE_PROFILE_RPT_5
...
df_5.createOrReplaceTempView("Shortcut_to_SITE_PROFILE_RPT_5")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_SALES_TRANS_SKU_6
...
df_6.createOrReplaceTempView("Shortcut_to_SALES_TRANS_SKU_6")
...
df_7 = spark.sql("""SELECT
  ...
  W.PRIMARY_SAP_DEPT_ID AS PRIMARY_SAP_DEPT_ID,
  CASE
    WHEN SUM(NVL(S.GROSS_SALES_LOC_AMT, 0)) = 0 THEN 0
    ELSE ROUND(
      SUM(NVL(W.SUM_EARNINGS_LOC_AMT, 0)) / SUM(NVL(S.GROSS_SALES_LOC_AMT, 0)),
      4
    )
  END AS MARGIN_RATE,
...
FROM
  (
    ...
    FROM
      (
        ...
        FROM
          Shortcut_to_EMPL_EMPL_LOC_WK_2 E
          JOIN EARNINGS_ID I ON E.EARN_ID = I.EARN_ID
          JOIN (
            SELECT
              ...
              PRIMARY_SAP_DEPT_ID
            FROM
              Shortcut_to_SERVICES_MARGIN_CTRL_1
          ) C ON E.STORE_DEPT_NBR = C.STORE_DEPT_NBR
          AND E.WEEK_DT BETWEEN C.START_EFF_DT
          AND C.END_EFF_DT
          JOIN SERVICES_MARGIN_WAGE_TYPE_CTRL WC ON I.PS_WAGE_TYPE_CD = WC.PS_WAGE_TYPE_CD
          AND E.WEEK_DT BETWEEN WC.START_EFF_DT
          AND WC.END_EFF_DT
        WHERE
  ...
  JOIN (
    ...
    FROM
      Shortcut_to_SALES_TRANS_SKU_6 S
      JOIN SKU_PROFILE_RPT K ON S.PRODUCT_ID = K.PRODUCT_ID
      JOIN DAYS D ON S.DAY_DT = D.DAY_DT
      JOIN (
        SELECT
          DISTINCT START_EFF_DT,
          END_EFF_DT,
        ...
        FROM
          Shortcut_to_SERVICES_MARGIN_CTRL_1
      ) C ON S.DAY_DT BETWEEN C.START_EFF_DT
      AND C.END_EFF_DT
