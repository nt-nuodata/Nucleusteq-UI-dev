# Databricks notebook source
# MAGIC %run "./udf_informatica"

# COMMAND ----------


from pyspark.sql.types import *

spark.sql("use DELTA_TRAINING")
spark.sql("set spark.sql.legacy.timeParserPolicy = LEGACY")


# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_EVENT_AMS_PRE_0


df_0 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  OFFER_AS_DISC_IND AS OFFER_AS_DISC_IND,
  VENDOR_FUNDED_IND AS VENDOR_FUNDED_IND,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  INSERT_FLAG AS INSERT_FLAG,
  DELETE_FLAG AS DELETE_FLAG
FROM
  MA_EVENT_AMS_PRE""")

df_0.createOrReplaceTempView("Shortcut_to_MA_EVENT_AMS_PRE_0")

# COMMAND ----------
# DBTITLE 1, SQ_Shortcut_to_MA_EVENT_AMS_PRE_1


df_1 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  Shortcut_to_MA_EVENT_AMS_PRE_0
WHERE
  Shortcut_to_MA_EVENT_AMS_PRE_0.INSERT_FLAG = 1""")

df_1.createOrReplaceTempView("SQ_Shortcut_to_MA_EVENT_AMS_PRE_1")

# COMMAND ----------
# DBTITLE 1, EXP_DeriveLoadFlag_2


df_2 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  'I' AS LOAD_FLG,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  SQ_Shortcut_to_MA_EVENT_AMS_PRE_1""")

df_2.createOrReplaceTempView("EXP_DeriveLoadFlag_2")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_EVENT_AMS_PRE1_3


df_3 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  OFFER_AS_DISC_IND AS OFFER_AS_DISC_IND,
  VENDOR_FUNDED_IND AS VENDOR_FUNDED_IND,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  INSERT_FLAG AS INSERT_FLAG,
  DELETE_FLAG AS DELETE_FLAG
FROM
  MA_EVENT_AMS_PRE""")

df_3.createOrReplaceTempView("Shortcut_to_MA_EVENT_AMS_PRE1_3")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_EVENT_4


df_4 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  SAP_DEPT_ID AS SAP_DEPT_ID,
  PRODUCT_ID AS PRODUCT_ID,
  COUNTRY_CD AS COUNTRY_CD,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  LOCATION_ID AS LOCATION_ID,
  MOVEMENT_ID AS MOVEMENT_ID,
  VALUATION_CLASS_CD AS VALUATION_CLASS_CD,
  GL_ACCT_NBR AS GL_ACCT_NBR,
  LOCATION_TYPE_ID AS LOCATION_TYPE_ID,
  ROYALTY_BRAND_ID AS ROYALTY_BRAND_ID,
  BRAND_CD AS BRAND_CD,
  MA_FORMULA_CD AS MA_FORMULA_CD,
  FISCAL_MO AS FISCAL_MO,
  SAP_CATEGORY_ID AS SAP_CATEGORY_ID,
  FROM_LOCATION_ID AS FROM_LOCATION_ID,
  SOURCE_VENDOR_ID AS SOURCE_VENDOR_ID,
  COMPANY_ID AS COMPANY_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  EM_VENDOR_FUNDING_ID AS EM_VENDOR_FUNDING_ID,
  EM_COMMENT AS EM_COMMENT,
  EM_BILL_ALT_VENDOR_FLAG AS EM_BILL_ALT_VENDOR_FLAG,
  EM_ALT_VENDOR_ID AS EM_ALT_VENDOR_ID,
  EM_ALT_VENDOR_NAME AS EM_ALT_VENDOR_NAME,
  EM_ALT_VENDOR_COUNTRY_CD AS EM_ALT_VENDOR_COUNTRY_CD,
  EM_VENDOR_ID AS EM_VENDOR_ID,
  EM_VENDOR_NAME AS EM_VENDOR_NAME,
  EM_VENDOR_COUNTRY_CD AS EM_VENDOR_COUNTRY_CD,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  UPDATE_DT AS UPDATE_DT,
  LOAD_DT AS LOAD_DT
FROM
  MA_EVENT""")

df_4.createOrReplaceTempView("Shortcut_to_MA_EVENT_4")

# COMMAND ----------
# DBTITLE 1, SQ_Shortcut_to_MA_EVENT_5


df_5 = spark.sql("""SELECT
  Shortcut_to_MA_EVENT_4.MA_EVENT_ID AS MA_EVENT_ID,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.OFFER_ID AS OFFER_ID,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.START_DT AS START_DT,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.END_DT AS END_DT,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.MA_EVENT_DESC AS MA_EVENT_DESC,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.MA_PCT_IND AS MA_PCT_IND,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.MA_AMT AS MA_AMT,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.MA_MAX_AMT AS MA_MAX_AMT,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.VENDOR_FUNDED_IND AS VENDOR_FUNDED_IND,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.DELETE_FLAG AS DELETE_FLAG,
  Shortcut_to_MA_EVENT_AMS_PRE1_3.OFFER_AS_DISC_IND AS OFFER_AS_DISC_IND,
  Shortcut_to_MA_EVENT_4.LOAD_DT AS LOAD_DT,
  Shortcut_to_MA_EVENT_4.START_DT AS tgt_START_DT,
  Shortcut_to_MA_EVENT_4.END_DT AS tgt_END_DT,
  Shortcut_to_MA_EVENT_4.MA_PCT_IND AS tgt_MA_PCT_IND,
  Shortcut_to_MA_EVENT_4.MA_AMT AS tgt_MA_AMT,
  Shortcut_to_MA_EVENT_4.MA_MAX_AMT AS tgt_MA_MAX_AMT,
  Shortcut_to_MA_EVENT_4.MA_EVENT_DESC AS tgt_MA_EVENT_DESC,
  Shortcut_to_MA_EVENT_4.VENDOR_NAME_TXT AS tgt_VENDOR_NAME_TXT,
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  Shortcut_to_MA_EVENT_4,
  Shortcut_to_MA_EVENT_AMS_PRE1_3
WHERE
  Shortcut_to_MA_EVENT_4.MA_EVENT_ID = Shortcut_to_MA_EVENT_AMS_PRE1_3.MA_EVENT_ID
  AND Shortcut_to_MA_EVENT_AMS_PRE1_3.INSERT_FLAG = 0
  AND Shortcut_to_MA_EVENT_4.MA_EVENT_TYPE_ID = 2
  AND Shortcut_to_MA_EVENT_AMS_PRE1_3.MA_EVENT_SOURCE_ID = 6""")

df_5.createOrReplaceTempView("SQ_Shortcut_to_MA_EVENT_5")

# COMMAND ----------
# DBTITLE 1, EXP_DeriveColumns_6


df_6 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  DECODE(
    TRUE,
    VENDOR_FUNDED_IND = 0
    OR DELETE_FLAG = 1
    OR OFFER_AS_DISC_IND = 0,
    'D',
    IFF(
      ISNULL(START_DT),
      TO_DATE('12319999', 'MMDDYYYY'),
      START_DT
    ) <> IFF(
      ISNULL(tgt_START_DT),
      TO_DATE('12319999', 'MMDDYYYY'),
      tgt_START_DT
    )
    OR IFF(
      ISNULL(END_DT),
      TO_DATE('12319999', 'MMDDYYYY'),
      END_DT
    ) <> IFF(
      ISNULL(tgt_END_DT),
      TO_DATE('12319999', 'MMDDYYYY'),
      tgt_END_DT
    )
    OR IFF(ISNULL(MA_PCT_IND), 9, MA_PCT_IND) <> IFF(ISNULL(tgt_MA_PCT_IND), 9, tgt_MA_PCT_IND)
    OR IFF(ISNULL(MA_AMT), 999999, MA_AMT) <> IFF(ISNULL(tgt_MA_AMT), 999999, tgt_MA_AMT),
    'R',
    IFF(ISNULL(MA_EVENT_DESC), ' ', MA_EVENT_DESC) <> IFF(ISNULL(tgt_MA_EVENT_DESC), ' ', tgt_MA_EVENT_DESC)
    OR IFF(ISNULL(VENDOR_NAME_TXT), ' ', VENDOR_NAME_TXT) <> IFF(
      ISNULL(tgt_VENDOR_NAME_TXT),
      ' ',
      tgt_VENDOR_NAME_TXT
    )
    OR IFF(ISNULL(MA_MAX_AMT), 999999, MA_MAX_AMT) <> IFF(ISNULL(tgt_MA_MAX_AMT), 999999, tgt_MA_MAX_AMT),
    'U',
    'N'
  ) AS LOAD_FLAG,
  tgt_MA_MAX_AMT AS tgt_MA_MAX_AMT,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  SQ_Shortcut_to_MA_EVENT_5""")

df_6.createOrReplaceTempView("EXP_DeriveColumns_6")

# COMMAND ----------
# DBTITLE 1, FIL_NoChanges_7


df_7 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  LOAD_FLAG AS LOAD_FLAG,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  EXP_DeriveColumns_6
WHERE
  LOAD_FLAG <> 'N'""")

df_7.createOrReplaceTempView("FIL_NoChanges_7")

# COMMAND ----------
# DBTITLE 1, Union_8


df_8 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  LOAD_FLAG AS LOAD_FLG,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  FIL_NoChanges_7
UNION ALL
SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  LOAD_FLG AS LOAD_FLG,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  EXP_DeriveLoadFlag_2""")

df_8.createOrReplaceTempView("Union_8")

# COMMAND ----------
# DBTITLE 1, EXP_SetLoadDt2_9


df_9 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  LOAD_FLG AS LOAD_FLG,
  now() AS CURRENT_TSTMP,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  Union_8""")

df_9.createOrReplaceTempView("EXP_SetLoadDt2_9")

# COMMAND ----------
# DBTITLE 1, UPD_InsertDeleteOrUpdate_10


df_10 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  LOAD_FLG AS LOAD_FLG,
  CURRENT_TSTMP AS LOAD_DT,
  CURRENT_TSTMP AS UPDATE_DT,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id,
  DECODE(
    TRUE,
    LOAD_FLG = 'I',
    'DD_INSERT',
    LOAD_FLG = 'D',
    'DD_DELETE',
    'DD_UPDATE'
  ) AS UPDATE_STRATEGY_FLAG
FROM
  EXP_SetLoadDt2_9""")

df_10.createOrReplaceTempView("UPD_InsertDeleteOrUpdate_10")

# COMMAND ----------
# DBTITLE 1, FIL_LoadFlag_11


df_11 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  MA_MAX_AMT AS VENDOR_NAME_TXT,
  LOAD_FLG AS LOAD_FLG,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  Union_8
WHERE
  LOAD_FLG = 'D'
  OR LOAD_FLG = 'R'
  OR LOAD_FLG = 'I'""")

df_11.createOrReplaceTempView("FIL_LoadFlag_11")

# COMMAND ----------
# DBTITLE 1, EXP_SetLoadDt_12


df_12 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  IFF(LOAD_FLG = 'R', 'U', LOAD_FLG) AS o_LOAD_FLG,
  TRUNC(now()) AS LOAD_DT,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  FIL_LoadFlag_11""")

df_12.createOrReplaceTempView("EXP_SetLoadDt_12")

# COMMAND ----------
# DBTITLE 1, UPD_InsertOnly_13


df_13 = spark.sql("""SELECT
  MA_EVENT_ID AS MA_EVENT_ID,
  OFFER_ID AS OFFER_ID,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  MA_PCT_IND AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  VENDOR_NAME_TXT AS VENDOR_NAME_TXT,
  o_LOAD_FLG AS o_LOAD_FLG,
  LOAD_DT AS LOAD_DT,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  EXP_SetLoadDt_12""")

df_13.createOrReplaceTempView("UPD_InsertOnly_13")

# COMMAND ----------
# DBTITLE 1, MA_EVENT


spark.sql("""MERGE INTO MA_EVENT AS TARGET
USING
  UPD_InsertDeleteOrUpdate_10 AS SOURCE ON TARGET.MA_EVENT_ID = SOURCE.MA_EVENT_ID
  WHEN MATCHED
  AND SOURCE.UPDATE_STRATEGY_FLAG = "DD_UPDATE" THEN
UPDATE
SET
  TARGET.MA_EVENT_ID = SOURCE.MA_EVENT_ID,
  TARGET.OFFER_ID = SOURCE.OFFER_ID,
  TARGET.START_DT = SOURCE.START_DT,
  TARGET.END_DT = SOURCE.END_DT,
  TARGET.MA_EVENT_TYPE_ID = SOURCE.MA_EVENT_TYPE_ID,
  TARGET.MA_EVENT_SOURCE_ID = SOURCE.MA_EVENT_SOURCE_ID,
  TARGET.MA_EVENT_DESC = SOURCE.MA_EVENT_DESC,
  TARGET.VENDOR_NAME_TXT = SOURCE.VENDOR_NAME_TXT,
  TARGET.MA_PCT_IND = SOURCE.MA_PCT_IND,
  TARGET.MA_AMT = SOURCE.MA_AMT,
  TARGET.MA_MAX_AMT = SOURCE.MA_MAX_AMT,
  TARGET.UPDATE_DT = SOURCE.UPDATE_DT,
  TARGET.LOAD_DT = SOURCE.LOAD_DT
  WHEN MATCHED
  AND SOURCE.UPDATE_STRATEGY_FLAG = "DD_DELETE"
  AND TARGET.OFFER_ID = SOURCE.OFFER_ID
  AND TARGET.START_DT = SOURCE.START_DT
  AND TARGET.END_DT = SOURCE.END_DT
  AND TARGET.MA_EVENT_TYPE_ID = SOURCE.MA_EVENT_TYPE_ID
  AND TARGET.MA_EVENT_SOURCE_ID = SOURCE.MA_EVENT_SOURCE_ID
  AND TARGET.MA_EVENT_DESC = SOURCE.MA_EVENT_DESC
  AND TARGET.VENDOR_NAME_TXT = SOURCE.VENDOR_NAME_TXT
  AND TARGET.MA_PCT_IND = SOURCE.MA_PCT_IND
  AND TARGET.MA_AMT = SOURCE.MA_AMT
  AND TARGET.MA_MAX_AMT = SOURCE.MA_MAX_AMT
  AND TARGET.UPDATE_DT = SOURCE.UPDATE_DT
  AND TARGET.LOAD_DT = SOURCE.LOAD_DT THEN DELETE
  WHEN NOT MATCHED
  AND SOURCE.UPDATE_STRATEGY_FLAG = "DD_INSERT" THEN
INSERT
  (
    TARGET.MA_EVENT_ID,
    TARGET.OFFER_ID,
    TARGET.START_DT,
    TARGET.END_DT,
    TARGET.MA_EVENT_TYPE_ID,
    TARGET.MA_EVENT_SOURCE_ID,
    TARGET.MA_EVENT_DESC,
    TARGET.VENDOR_NAME_TXT,
    TARGET.MA_PCT_IND,
    TARGET.MA_AMT,
    TARGET.MA_MAX_AMT,
    TARGET.UPDATE_DT,
    TARGET.LOAD_DT
  )
VALUES
  (
    SOURCE.MA_EVENT_ID,
    SOURCE.OFFER_ID,
    SOURCE.START_DT,
    SOURCE.END_DT,
    SOURCE.MA_EVENT_TYPE_ID,
    SOURCE.MA_EVENT_SOURCE_ID,
    SOURCE.MA_EVENT_DESC,
    SOURCE.VENDOR_NAME_TXT,
    SOURCE.MA_PCT_IND,
    SOURCE.MA_AMT,
    SOURCE.MA_MAX_AMT,
    SOURCE.UPDATE_DT,
    SOURCE.LOAD_DT
  )""")

# COMMAND ----------
# DBTITLE 1, MA_EVENT_RESTATE_HIST


spark.sql("""INSERT INTO
  MA_EVENT_RESTATE_HIST
SELECT
  LOAD_DT AS LOAD_DT,
  MA_EVENT_ID AS MA_EVENT_ID,
  NULL AS OFFER_ID,
  NULL AS SAP_DEPT_ID,
  NULL AS PRODUCT_ID,
  NULL AS COUNTRY_CD,
  START_DT AS START_DT,
  END_DT AS END_DT,
  MA_EVENT_TYPE_ID AS MA_EVENT_TYPE_ID,
  MA_EVENT_SOURCE_ID AS MA_EVENT_SOURCE_ID,
  NULL AS LOCATION_ID,
  NULL AS MOVEMENT_ID,
  NULL AS VALUATION_CLASS_CD,
  NULL AS GL_ACCT_NBR,
  NULL AS LOCATION_TYPE_ID,
  NULL AS ROYALTY_BRAND_ID,
  NULL AS BRAND_CD,
  NULL AS MA_FORMULA_CD,
  NULL AS FISCAL_MO,
  NULL AS SAP_CATEGORY_ID,
  NULL AS FROM_LOCATION_ID,
  NULL AS SOURCE_VENDOR_ID,
  NULL AS COMPANY_ID,
  MA_EVENT_DESC AS MA_EVENT_DESC,
  NULL AS EM_VENDOR_FUNDING_ID,
  NULL AS EM_COMMENT,
  NULL AS EM_BILL_ALT_VENDOR_FLAG,
  NULL AS EM_ALT_VENDOR_ID,
  NULL AS EM_ALT_VENDOR_NAME,
  NULL AS EM_ALT_VENDOR_COUNTRY_CD,
  NULL AS EM_VENDOR_ID,
  NULL AS EM_VENDOR_NAME,
  NULL AS EM_VENDOR_COUNTRY_CD,
  NULL AS VENDOR_NAME_TXT,
  NULL AS MA_PCT_IND,
  MA_AMT AS MA_AMT,
  MA_MAX_AMT AS MA_MAX_AMT,
  o_LOAD_FLG AS INS_UPD_DEL_FLAG FROMUPD_InsertOnly_13""")