...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_FISCAL_MO_CTRL_0
...
df_0.createOrReplaceTempView("Shortcut_to_MA_FISCAL_MO_CTRL_0")
...
FROM
  Shortcut_to_MA_FISCAL_MO_CTRL_0""")
...
FROM
  Shortcut_To_DAYS_2
WHERE
  Shortcut_To_DAYS_2.DAY_DT = CURRENT_DATE
  AND Shortcut_To_DAYS_2.FISCAL_DAY_OF_MO_NBR = 7""")
...
df_4 = spark.sql("""SELECT
  DAY_DT AS RESTATE_DT,
  IFF(FISCAL_MO_NBR = 1, FISCAL_MO - 89, FISCAL_MO - 1) AS FISCAL_MO,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
...
FROM
  ...
  RIGHT JOIN EXP_FISCAL_MO_4 DETAIL ON MASTER.FISCAL_MO = DETAIL.FISCAL_MO""")
...
df_6 = spark.sql("""SELECT
  ...
  IFF (ISNULL(FISCAL_MO1), 'I', 'U') AS INS_UPD_FLAG,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  JNR_FISCAL_MO_5""")
...
spark.sql("""MERGE INTO MA_FISCAL_MO_CTRL AS TARGET
USING
  UPD_STRATEGY_7 AS SOURCE ON TARGET.FISCAL_MO = SOURCE.FISCAL_MO
  WHEN MATCHED
  AND SOURCE.UPDATE_STRATEGY_FLAG = "DD_UPDATE" THEN
UPDATE
SET
  TARGET.FISCAL_MO = SOURCE.FISCAL_MO,
  TARGET.RESTATE_DT = SOURCE.RESTATE_DT,
  TARGET.UPDATE_TSTMP = SOURCE.UPDATE_TSTMP,
  TARGET.LOAD_TSTMP = SOURCE.LOAD_TSTMP
  WHEN MATCHED
  AND SOURCE.UPDATE_STRATEGY_FLAG = "DD_DELETE"
  AND TARGET.RESTATE_DT = SOURCE.RESTATE_DT
