...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_EVENT_0
...
df_0.createOrReplaceTempView("Shortcut_to_MA_EVENT_0")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_EM_EVENT_1
...
df_1.createOrReplaceTempView("Shortcut_to_EM_EVENT_1")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_MOVEMENT_DAY_2
...
df_2.createOrReplaceTempView("Shortcut_to_MA_MOVEMENT_DAY_2")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_MOVEMENT_INFO_3
...
df_3.createOrReplaceTempView("Shortcut_to_MOVEMENT_INFO_3")
...
df_4 = spark.sql("""SELECT
  pre.day_dt AS DAY_DT,
  pre.product_id AS PRODUCT_ID,
  pre.location_id AS LOCATION_ID,
  pre.movement_id AS MOVEMENT_ID,
  pre.po_nbr AS PO_NBR,
  pre.po_line_nbr AS PO_LINE_NBR,
  pre.ma_event_id AS MA_EVENT_ID,
  pre.sto_type_id AS STO_TYPE_ID,
  pre.ma_trans_amt AS MA_TRANS_AMT,
  pre.ma_trans_cost AS MA_TRANS_COST,
  pre.ma_trans_qty AS MA_TRANS_QTY,
  pre.exch_rate_pct AS EXCH_RATE_PCT,
  CURRENT_DATE AS UPDATE_DT,
  NVL(mmd.load_dt, CURRENT_DATE) AS LOAD_DT,
  CASE
...
FROM
  (
    ...
    FROM
      movement_day md
      JOIN Shortcut_to_MOVEMENT_INFO_3 mi ON md.movement_id = mi.movement_id
      JOIN ma_event e ON md.day_dt BETWEEN e.start_dt
      AND e.end_dt
      AND md.product_id = e.product_id
      JOIN site_profile sp ON md.location_id = sp.location_id
      AND NVL(e.country_cd, sp.country_cd) = sp.country_cd
      JOIN sku_profile sku ON md.product_id = sku.product_id
      LEFT JOIN USR_STORE_ATTRIBUTES USA ON md.LOCATION_ID = USA.LOCATION_ID
    WHERE
      e.ma_event_source_id = 5
      AND sp.location_type_id = 8
      AND USA.VET_TYPE_ID IN (1, 2)
    ...
    FROM
      movement_day md
      JOIN sku_profile sku ON md.product_id = sku.product_id
      JOIN site_profile site ON md.location_id = site.location_id
      JOIN ma_event e ON md.day_dt BETWEEN e.start_dt
      AND e.end_dt
  ...
  LEFT OUTER JOIN Shortcut_to_MA_MOVEMENT_DAY_2 mmd ON pre.day_dt = mmd.day_dt
...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_SITE_PROFILE_6
...
df_6.createOrReplaceTempView("Shortcut_to_SITE_PROFILE_6")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_MOVEMENT_DAY_7
...
df_7.createOrReplaceTempView("Shortcut_to_MOVEMENT_DAY_7")
...
FROM
  SQ_Shortcut_to_MA_MOVEMENT_DAY_4""")