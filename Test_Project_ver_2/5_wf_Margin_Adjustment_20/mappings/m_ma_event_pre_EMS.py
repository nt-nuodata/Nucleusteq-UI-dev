...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_EVENT_TYPE_1
...
df_1.createOrReplaceTempView("Shortcut_to_MA_EVENT_TYPE_1")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_EVENT1_2
...
df_2.createOrReplaceTempView("Shortcut_to_MA_EVENT1_2")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_EVENT_3
...
df_3.createOrReplaceTempView("Shortcut_to_MA_EVENT_3")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_EM_EVENT1_4
...
df_4.createOrReplaceTempView("Shortcut_to_EM_EVENT1_4")

# COMMAND ----------
# DBTITLE 1, ASQ_MA_EVENT_PRE_NEW_5


df_5 = spark.sql("""SELECT
  vf.product_id AS PRODUCT_ID,
  vf.country_cd AS COUNTRY_CD,
  vf.start_dt AS START_DT,
  vf.end_dt AS END_DT,
  met.ma_event_type_id AS MA_EVENT_TYPE_ID,
  1 AS MA_EVENT_SOURCE_ID,
  d.ma_event_desc AS MA_EVENT_DESC,
  vf.em_vendor_funding_id AS EM_VENDOR_FUNDING_ID,
  vf.em_comment AS EM_COMMENT,
  vf.bill_alternate_vendor_flag AS EM_BILL_ALT_VENDOR,
  vf.alternate_vendor_id AS EM_ALT_VENDOR_ID,
  CASE
    WHEN vf.alternate_vendor_id IS NOT NULL THEN NVL(av.vendor_name, 'UNKNOWN')
  END AS EM_ALT_VENDOR_NAME,
  av.country_cd AS EM_ALT_VENDOR_COUNTRY_CD,
  es.vendor_id AS EM_VENDOR_ID,
  es.vendor_name AS EM_VENDOR_NAME,
  v.country_cd AS EM_VENDOR_COUNTRY_CD,
  vf.vf_amt AS MA_AMT,
  CASE
...
FROM
  ...
  JOIN Shortcut_to_EM_EVENT1_4 e ON vf.em_event_id = e.em_event_id
  JOIN em_event_sku es ON vf.em_event_id = es.em_event_id
  AND vf.product_id = es.product_id
  AND vf.country_cd = es.country_cd
  JOIN ma_event_type met ON met.em_tpr_type = vf.vf_type
  JOIN (
    SELECT
    ...
    FROM
      em_vendor_funding vf,
      Shortcut_to_EM_EVENT1_4 e,
      sku_profile sp
    WHERE
      vf.em_event_id = e.em_event_id
      AND vf.product_id = sp.product_id
  ) d ON vf.em_vendor_funding_id = d.em_vendor_funding_id
  LEFT JOIN (
    SELECT
    ...
    WHERE
      vendor_type_id = 2
  ) av ON RTRIM(LTRIM(vf.alternate_vendor_id)) = av.vendor_id
  LEFT JOIN (
    SELECT
      vendor_id,
      vendor_name,
...
WHERE
  ...
  AND vf.vf_amt > 0
  AND (vf.em_vendor_funding_id, vf.country_cd) NOT IN (
    SELECT
      DISTINCT em_vendor_funding_id,
      country_cd
    FROM
      Shortcut_to_MA_EVENT1_2
    WHERE
      ma_event_source_id = 1
  )""")

df_5.createOrReplaceTempView("ASQ_MA_EVENT_PRE_NEW_5")
...
df_6 = spark.sql("""SELECT
  ...
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
  ASQ_MA_EVENT_PRE_NEW_5""")

df_6.createOrReplaceTempView("EXP_NEW_6")

spark.sql("""UPDATE SEQ_MA_EVENT_ID SET CURRVAL = (SELECT MAX(MA_EVENT_ID) FROM EXP_NEW_6) , NEXTVAL = (SELECT MAX(MA_EVENT_ID) FROM EXP_NEW_6) + (SELECT Increment_By FROM EXP_NEW_6)""")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_EM_EVENT_7
...
df_7.createOrReplaceTempView("Shortcut_to_EM_EVENT_7")

# COMMAND ----------
# DBTITLE 1, ASQ_MA_EVENT_PRE_EXISTING_8


df_8 = spark.sql("""SELECT
  me.ma_event_id AS MA_EVENT_ID,
  vf.product_id AS PRODUCT_ID,
  vf.country_cd AS COUNTRY_CD,
  vf.start_dt AS START_DT,
  vf.end_dt AS END_DT,
...
FROM
  em_vendor_funding vf
  JOIN Shortcut_to_MA_EVENT_3 me ON vf.em_vendor_funding_id = me.em_vendor_funding_id
  AND vf.country_cd = me.country_cd
  JOIN Shortcut_to_EM_EVENT_7 e ON vf.em_event_id = e.em_event_id
  JOIN em_event_sku es ON vf.em_event_id = es.em_event_id
  AND vf.product_id = es.product_id
  AND vf.country_cd = es.country_cd
  JOIN ma_event_type met ON met.em_tpr_type = vf.vf_type
...
df_8.createOrReplaceTempView("ASQ_MA_EVENT_PRE_EXISTING_8")
...
df_9.createOrReplaceTempView("UNI_EMS_9")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_EM_VENDOR_FUNDING_10
...
df_10.createOrReplaceTempView("Shortcut_to_EM_VENDOR_FUNDING_10")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_SKU_PROFILE_11
...
df_11.createOrReplaceTempView("Shortcut_to_SKU_PROFILE_11")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_EM_EVENT_SKU_12
...
df_12.createOrReplaceTempView("Shortcut_to_EM_EVENT_SKU_12")
