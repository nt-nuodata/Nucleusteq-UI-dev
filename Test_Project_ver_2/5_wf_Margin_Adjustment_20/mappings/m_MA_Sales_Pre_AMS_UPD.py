...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_SALES_PRE_1
...
df_1.createOrReplaceTempView("Shortcut_to_MA_SALES_PRE_1")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_EVENT_2
...
df_2.createOrReplaceTempView("Shortcut_to_MA_EVENT_2")
    ...
    SELECT
      ...
      ROW_NUMBER() OVER (
        PARTITION BY
          ...
          OFFER_ID
        ORDER BY
          (ABS(OFFER_AMT) - ABS(MA_SALES_AMT)) DESC
      ) AS RNK
    FROM
      (
        SELECT
          ...
          (
            (
              CASE
                WHEN MA_PCT_IND = 1 THEN ROUND(
                  SUM(ABS(SO.OFFER_AMT)) OVER (
                    PARTITION BY
        ...
        FROM
          Shortcut_to_MA_SALES_PRE_1 MS,
          Shortcut_to_MA_EVENT_2 ME,
          (
            SELECT
              DISTINCT DAY_DT,
              SALES_INSTANCE_ID,
              ...
              UPC_ID,
              POS_TXN_SEQ_NBR,
              COUNT(DISTINCT SCAN_SEQ_NBR) OVER (
                PARTITION BY
                  DAY_DT,
                  SALES_INSTANCE_ID,
                  OFFER_ID
            ...
            FROM
              Shortcut_to_SALES_TRANS_UPC_OFFER_0
            WHERE
              DAY_DT >= (
                SELECT
                  MIN(DAY_DT)
                FROM
                  Shortcut_to_MA_SALES_PRE_1
              )
          ) SO
        WHERE
...
spark.sql("""MERGE INTO MA_SALES_PRE AS TARGET
USING
  UPD_UpdateOnly_4 AS SOURCE ON TARGET.LOCATION_ID = SOURCE.LOCATION_ID
  AND TARGET.DAY_DT = SOURCE.DAY_DT
  AND TARGET.COMBO_TYPE_CD = SOURCE.COMBO_TYPE_CD
  AND TARGET.PARENT_UPC_ID = SOURCE.PARENT_UPC_ID
  AND TARGET.POS_TXN_SEQ_NBR = SOURCE.POS_TXN_SEQ_NBR
  AND TARGET.UPC_ID = SOURCE.UPC_ID
  AND TARGET.TP_INVOICE_NBR = SOURCE.TP_INVOICE_NBR
...
SET
  ...
  TARGET.TP_INVOICE_NBR = SOURCE.TP_INVOICE_NBR,
  TARGET.PARENT_UPC_ID = SOURCE.PARENT_UPC_ID,
  TARGET.COMBO_TYPE_CD = SOURCE.COMBO_TYPE_CD,
  TARGET.POS_TXN_SEQ_NBR = SOURCE.POS_TXN_SEQ_NBR,
  TARGET.MA_EVENT_ID = SOURCE.MA_EVENT_ID,
  TARGET.MA_SALES_AMT = SOURCE.MA_SALES_AMT""")