...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_MA_CASH_DISCOUNT_CTRL_0
...
df_0.createOrReplaceTempView("Shortcut_to_MA_CASH_DISCOUNT_CTRL_0")
...
FROM
  Shortcut_to_MA_CASH_DISCOUNT_CTRL_0""")

df_1.createOrReplaceTempView("SQ_Shortcut_to_MA_CASH_DISCOUNT_CTRL_1")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_VENDOR_PROFILE_RPT_2
...
df_2.createOrReplaceTempView("Shortcut_to_VENDOR_PROFILE_RPT_2")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_SALES_DAY_SKU_STORE_RPT_3
...
df_3.createOrReplaceTempView("Shortcut_to_SALES_DAY_SKU_STORE_RPT_3")
...
df_4 = spark.sql("""SELECT
  DISTINCT V.VENDOR_ID AS VENDOR_ID,
  SUBSTR(
    V.PAYMENT_TERM_DESC,
    1,
    INSTR(V.PAYMENT_TERM_DESC, '%') - 1
  ) AS TXT_EST_CASH_DISCOUNT_PCT,
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  Shortcut_to_VENDOR_PROFILE_RPT_2 V
  JOIN Shortcut_to_SALES_DAY_SKU_STORE_RPT_3 S ON V.VENDOR_ID = S.VENDOR_ID
WHERE
...
df_5 = spark.sql("""SELECT
  VENDOR_ID AS VENDOR_ID,
  IFF(
    IS_NUMBER(TXT_EST_CASH_DISCOUNT_PCT),
    TO_DECIMAL(TXT_EST_CASH_DISCOUNT_PCT) *.01,
    0
  ) AS THIS_EST_CASH_DISCOUNT_PCT,
  1 AS FLAG,
  Monotonically_Increasing_Id AS Monotonically_Increasing_Id
FROM
...
FROM
  Shortcut_To_DAYS_7
WHERE
  Shortcut_To_DAYS_7.DAY_DT = CURRENT_DATE - 1""")
...
FROM
  ...
  INNER JOIN EXP_FLAG_9 DETAIL ON MASTER.FLAG = DETAIL.FLAG""")
...
FROM
  ...
  LEFT JOIN SQ_Shortcut_to_MA_CASH_DISCOUNT_CTRL_1 DETAIL ON MASTER.VENDOR_ID = DETAIL.SOURCE_VENDOR_ID
  AND MASTER.FISCAL_MO = DETAIL.FISCAL_MO""")
...
df_12 = spark.sql("""SELECT
  ...
  IFF(
    ...
    'I',
    IFF(
      THIS_EST_CASH_DISCOUNT_PCT <> IFF(
        ISNULL(EST_CASH_DISCOUNT_PCT),
        0,
        EST_CASH_DISCOUNT_PCT
      ),
      'U',
      'X'
    )
...
USING
  UPD_STRATEGY_15 AS SOURCE ON TARGET.SOURCE_VENDOR_ID = SOURCE.VENDOR_ID
...
SET
  ...
  TARGET.SOURCE_VENDOR_ID = SOURCE.VENDOR_ID,
  TARGET.EST_CASH_DISCOUNT_PCT = SOURCE.THIS_EST_CASH_DISCOUNT_PCT,
  TARGET.UPDATE_TSTMP = SOURCE.UPDATE_TSTMP,
  TARGET.LOAD_TSTMP = SOURCE.LOAD_TSTMP
  WHEN MATCHED
  AND SOURCE.UPDATE_STRATEGY_FLAG = "DD_DELETE"
  AND TARGET.EST_CASH_DISCOUNT_PCT = SOURCE.THIS_EST_CASH_DISCOUNT_PCT
...
VALUES
  (
    SOURCE.FISCAL_MO,
    SOURCE.VENDOR_ID,
    SOURCE.THIS_EST_CASH_DISCOUNT_PCT,
    SOURCE.UPDATE_TSTMP,
    SOURCE.LOAD_TSTMP
  )""")