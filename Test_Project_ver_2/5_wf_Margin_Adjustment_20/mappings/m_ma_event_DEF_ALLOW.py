...
# COMMAND ----------
# DBTITLE 1, Shortcut_to_USR_MA_DEF_ALLOW_CTRL1_1
...
df_1.createOrReplaceTempView("Shortcut_to_USR_MA_DEF_ALLOW_CTRL1_1")

# COMMAND ----------
# DBTITLE 1, Shortcut_to_USR_MA_DEF_ALLOW_CTRL_2
...
df_2.createOrReplaceTempView("Shortcut_to_USR_MA_DEF_ALLOW_CTRL_2")
...
df_6 = spark.sql("""SELECT
  ...
  D.END_DT AS END_DT,
  80 AS MA_EVENT_TYPE_ID,
  13 AS MA_EVENT_SOURCE_ID,
  UMDAC.FISCAL_MO AS FISCAL_MO,
  UMDAC.SOURCE_VENDOR_ID AS SOURCE_VENDOR_ID,
  'Defective Allowance: FiscalMo=' || UMDAC.FISCAL_MO:: VARCHAR (10) || ': SourceVendor=' || UMDAC.SOURCE_VENDOR_ID:: VARCHAR (20) AS MA_EVENT_DESC,
  1 AS MA_PCT_IND,
...
FROM
  Shortcut_to_USR_MA_DEF_ALLOW_CTRL_2 UMDAC
  JOIN (
    SELECT
      FISCAL_MO,
      MIN(DAY_DT) START_DT,
    ...
    GROUP BY
      FISCAL_MO
  ) D ON UMDAC.FISCAL_MO = D.FISCAL_MO
  LEFT JOIN Shortcut_to_MA_EVENT1_3 M ON UMDAC.FISCAL_MO = M.FISCAL_MO
...
df_9 = spark.sql("""SELECT
  ...
  monotonically_increasing_id() AS Monotonically_Increasing_Id
FROM
  Shortcut_to_USR_MA_DEF_ALLOW_CTRL1_1 UMDAC
  JOIN Shortcut_to_MA_EVENT11_4 M ON UMDAC.FISCAL_MO = M.FISCAL_MO
  AND UMDAC.SOURCE_VENDOR_ID = M.SOURCE_VENDOR_ID
  AND M.MA_EVENT_SOURCE_ID = 13
WHERE
  NEW_MA_AMT <> M.MA_AMT
